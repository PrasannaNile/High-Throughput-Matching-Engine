...
namespace MatchingEngine {
    void Router::run() {
        while(true) {
            ...
            MatchingEngine::Order incomingOrder = {orderIdCtr, 0, orderSide, MatchingEngine::OrderStateEnum::NEW, 
                                                orderQuantity, orderPrice, 100000};
            orderIdCtr++;

            book.processOrder(incomingOrder);
            book.displayBuyers();
            book.displaySellers();
        }
    }
    ...
    std::vector<std::string> Router::tokenization(std::string& orderStr) {
    ...
    bool Router::handleInputException(std::vector<std::string>& orderToken, OrderSideEnum& orderSide, uint32_t& orderQuantity, uint64_t& orderPrice) {
